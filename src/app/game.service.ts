...
game() {
  setTimeout(() => {
    ...
    this.interval = setInterval(() => {      
        while(this.idx.length < 3){
        var rand = Math.floor(Math.random() * this.holes.length);
        if(this.idx.indexOf(rand) === -1) this.idx.unshift(rand);
        }
        this.holes.forEach((element, index) => {
        this.idx.indexOf(index) > -1
        ? (this.holes[this.idx[0]].moleOrHole = Boolean(false), this.holes[this.idx[1]].moleOrHole = Boolean(Math.random() < .6) , this.holes[this.idx[1]].moleOrHole = Boolean(Math.random() < .8) )
  ...
  randomHole() {
    ...
    mole() {
      const time = this.randomTime(3000, 4000);
      this.hole = this.randomHole();
      this.holes[this.idx2[0]].moleOrHole = Boolean(false)
      setTimeout(() => {
      this.holes[this.idx2[0]].moleOrHole = Boolean(true)
      if (this.__timer.remainingTime > 0)  
      this.mole();
      }, time);
      }
  startGame() {
      ...
      startGame2() {
        ...
        mole1() {
          ...
          this.holes[this.idx[0]].moleOrHole = Boolean(Math.random() < .8)
          setTimeout(() => {
          ...
          mole2() {
            ...
            this.holes[this.idx2[0]].moleOrHole = Boolean(Math.random() < .8)
            setTimeout(() => {
            this.holes[this.idx2[0]].moleOrHole = Boolean(true)
            if (this.__timer.remainingTime > 0)  
            this.mole2();
